...
namespace QuizGame
{
     static class Commends
    {
        ...
        /// <summary>
        /// Sprawdza czy ciąg znaków jest liczbą oraz czy podana cyfra znajduje się w przedziale oraz daje możliwość 
        /// zwrócenia konkretnej liczby poza przedziałem
        /// </summary>
        /// <param name="write">Jakie pytanie ma program zadać</param>
        /// <param name="minValue">Najmniejsza wartość jaką można podać</param>
        /// <param name="maxValue">Największa wartość jaką można podać</param>
        /// <returns></returns>
        public static int Checking(string write, int minValue, int maxValue, int outOfTheEquation)
        {
            int number = 0;
            bool var_Correct = true;
            string check;
            ...
            while (var_Correct)
            {
                ...
                if (Int32.TryParse(check, out number))
                {
                    ...
                    else if (number == outOfTheEquation)
                    {
                        var_Correct = false;
                    }
                    else
